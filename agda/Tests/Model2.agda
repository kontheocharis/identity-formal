...
record TT-Logic : Set2 where
  ...
  field
    ...
    _[_]C : ∀ {ΓL ΔL} → TmC ΔL → SubL ΓL ΔL → TmC ΓL
  ...
  field
    ...
    -- Computational context extension
    _▷C : ConL → ConL
    pC : ∀ {ΓL} → SubL (ΓL ▷C) ΓL
    qC : ∀ {ΓL} → TmC (ΓL ▷C)
    _,C_ : ∀ {ΓL ΔL} → (σ : SubL ΓL ΔL) → TmC ΓL → SubL ΓL (ΔL ▷C)
    ,∘C : ∀ {ΓL ΔL ΘL} {σ : SubL ΓL ΔL} {σ' : SubL ΘL ΓL} {t} → (σ ,C t) ∘L σ' ≡ (σ ∘L σ') ,C (t [ σ' ]C)
    pC,qC : ∀ {ΓL} → (pC {ΓL} ,C qC) ≡ idL
    pC∘, : ∀ {ΓL ΔL} {σ : SubL ΓL ΔL} {t : TmC ΓL} → pC ∘L (σ ,C t) ≡ σ
    qC[,] : ∀ {ΓL ΔL} {σ : SubL ΓL ΔL} {t : TmC ΓL} → qC [ σ ,C t ]C ≡ t
  ...
  ⟨_⟩C : ∀ {ΓC} → TmC ΓC → SubL ΓC (ΓC ▷C)
  ⟨ t ⟩C = idL ,C t
 
  _⁺C : ∀ {ΓC ΔC} → SubL ΓC ΔC → SubL (ΓC ▷C) (ΔC ▷C)
  σ ⁺C = (σ ∘L pC) ,C qC
  ...
  field
    ...
    -- Computational terms
    lamC : ∀ {ΓC} → TmC (ΓC ▷C) → TmC ΓC
    lamC[] : ∀ {ΓC ΔC t} {σ : SubL ΔC ΓC} → (lamC {ΓC} t) [ σ ]C ≡ lamC (t [ σ ⁺C ]C)

    appC : ∀ {ΓC} → TmC ΓC → TmC ΓC → TmC ΓC
    appC[] : ∀ {ΓC ΔC t u} {σ : SubL ΔC ΓC} → (appC {ΓC} t u) [ σ ]C ≡ appC (t [ σ ]C) (u [ σ ]C)
 
    -- Unit
    unit : ∀ {ΓC} → TmC ΓC
    unit[] : ∀ {ΓC ΔC} {σ : SubL ΔC ΓC} → (unit {ΓC}) [ σ ]C ≡ unit
    ...
    succC : ∀ {ΓC} → TmC ΓC → TmC ΓC
    succC[] : ∀ {ΓC ΔC t} {σ : SubL ΔC ΓC} → (succC {ΓC} t) [ σ ]C ≡ succC (t [ σ ]C)
 
    recC : ∀ {ΓC} → TmC ΓC → TmC (ΓC ▷C) → TmC ΓC → TmC ΓC
    recC[] : ∀ {ΓC ΔC z s n} {σ : SubL ΔC ΓC}
      → (recC {ΓC} z s n) [ σ ]C ≡ recC (z [ σ ]C) (s [ σ ⁺C ]C) (n [ σ ]C)
...
record TT : Set2 where
  ...
  field
    ...
    ConCC : ConL → Set
    ∙CC : ConCC ∙L
    _▷CC : ∀ {ΓC} → (ΓCC : ConCC ΓC) → ConCC (ΓC ▷C)

    Con : ConL → {ΓC : ConL} → ConCC ΓC → Set1
    ∙ : Con ∙L ∙CC
    _▷_ : ∀ {ΓL ΓC ΓCC} → Con ΓL {ΓC} ΓCC → (TL : Ty ΓL) → Con (ΓL ▷L TL) (ΓCC ▷CC)
    _▷0_ : ∀ {ΓL ΓC ΓCC} → Con ΓL {ΓC} ΓCC → (TL : Ty ΓL) → Con (ΓL ▷L TL) ΓCC

    Tm : ∀ {ΓL ΓC ΓCC} → Con ΓL {ΓC} ΓCC → (TL : Ty ΓL) → TmL ΓL TL → TmC ΓC → Set
    
    -- Terms
    
    -- Variables
    q : ∀ {ΓL ΓC ΓCC AL Γ} → Tm {ΓL ▷L AL} {ΓC ▷C} {ΓCC ▷CC} (Γ ▷ AL) (AL [ pL ]T) qL qC
    _[p] : ∀ {ΓL ΓC ΓCC AL Γ aL aC}
      → Tm Γ AL aL aC → Tm {ΓL ▷L AL} {ΓC ▷C} {ΓCC ▷CC} (Γ ▷ AL) (AL [ pL ]T) (aL [ pL ]L) (aC [ pC ]C)

    -- Functions
    lam : ∀ {ΓL ΓC ΓCC TL UL tL tC Γ}
      → Tm {ΓL ▷L TL} {ΓC ▷C} {ΓCC ▷CC} (Γ ▷ TL) UL tL tC → Tm Γ (Π TL UL) (lamL tL) (lamC tC)
    app : ∀ {ΓL ΓC ΓCC TL UL fL fC tL tC Γ}
      → Tm {ΓL} {ΓC} {ΓCC} Γ (Π TL UL) fL fC → Tm Γ TL tL tC
      → Tm Γ (UL [ ⟨ tL ⟩L ]T) (apL fL [ ⟨ tL ⟩L ]L) (appC fC tC)
      
    -- Natural numbers
    zero : ∀ {ΓL ΓC ΓCC Γ} → Tm {ΓL} {ΓC} {ΓCC} Γ Nat zeroL zeroC
    succ : ∀ {ΓL ΓC ΓCC Γ nL nC} → Tm {ΓL} {ΓC} {ΓCC} Γ Nat nL nC → Tm Γ Nat (succL nL) (succC nC)
    rec : ∀ {ΓL ΓC ΓCC P zL zC sL sC nL nC Γ}
      → Tm {ΓL} {ΓC} {ΓCC} Γ Nat nL nC
      → Tm Γ P zL zC
      → Tm (Γ ▷ P) (P [ pL ]T) sL sC
      → Tm Γ P (recL P zL sL nL) (recC zC sC nC) 

    -- Specialisations
    spec : ∀ {ΓL ΓC ΓCC Γ T aL aC}
      → Tm {ΓL} {ΓC} {ΓCC} Γ T aL aC
      → Tm Γ (Spec T {!   !}) (specL aL) unit
    unspec : ∀ {ΓL ΓC ΓCC Γ T aC aL aC'}
      → Tm {ΓL} {ΓC} {ΓCC} Γ (Spec T aC) aL aC'
      → Tm Γ T (unspecL aL) {!   !}
