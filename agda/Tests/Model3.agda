...
record TT : Set2 where
  ...
  field
    ...
    -- anything.
    Con : (ΓL : ConL) → (ΓC : ConL) → SubL ΓL ΓC → Set1
    Tm : ∀ {ΓL ΓC σ} → Con ΓL ΓC σ → (TL : Ty ΓL) → TmL ΓL TL → TmL ΓC Λ → Set
    
    -- Context
    ∙ : Con ∙L ∙L idL
    _▷_ : ∀ {ΓL ΓC σ} → Con ΓL ΓC σ → (TL : Ty ΓL) → Con (ΓL ▷L TL) (ΓC ▷L Λ) ((σ ∘L pL) ,L {!   !})
    _▷0_ : ∀ {ΓL ΓC σ} → Con ΓL ΓC σ → (TL : Ty ΓL) → Con (ΓL ▷L TL) ΓC (σ ∘L pL)
    
    -- Terms
    
    -- Variables
    q : ∀ {ΓL ΓC AL Γ} → Tm {ΓL ▷L AL} {ΓC ▷L Λ} (Γ ▷ AL) (AL [ pL ]T) qL (coeL Λ[] qL)
    _[p] : ∀ {ΓL ΓC AL Γ aL aC}
      → Tm Γ AL aL aC → Tm {ΓL ▷L AL} {ΓC ▷L Λ} (Γ ▷ AL) (AL [ pL ]T) (aL [ pL ]L) (coeL Λ[] (aC [ pL ]L))

    -- Functions
    lam : ∀ {ΓL ΓC TL UL tL tC Γ}
      → Tm {ΓL ▷L TL} {ΓC ▷L Λ} (Γ ▷ TL) UL tL tC → Tm Γ (Π TL UL) (lamL tL) (lamΛ tC)
    app : ∀ {ΓL ΓC TL UL fL fC tL tC Γ}
      → Tm {ΓL} {ΓC} Γ (Π TL UL) fL fC → Tm Γ TL tL tC
      → Tm Γ (UL [ ⟨ tL ⟩L ]T) (apL fL [ ⟨ tL ⟩L ]L) (appΛ fC tC)
    ...
    --   → Tm {ΓL} {ΓC} Γ P zL zC
    --   → Tm {ΓL ▷L P} {ΓC ▷L Λ} (Γ ▷ P) (P [ pL ]T) sL sC
    --   → Tm {ΓL} {ΓC} Γ P (recL P zL sL nL) (recC zC sC nC) 

    -- Specialisations
    spec : ∀ {ΓL ΓC σ Γ T aL aC} → Tm {ΓL} {ΓC} {σ} Γ T aL aC → Tm Γ (Spec T (coeL Λ[] (aC [ σ ]L))) (specL aL) aC
    unspec : ∀ {ΓL ΓC σ Γ T aC aL aC'} → Tm {ΓL} {ΓC} {σ} Γ (Spec T (coeL Λ[] (aC [ σ ]L))) aL aC' → Tm Γ T (unspecL aL) aC
