...
module OverPCA+ (A : PCA) (A+ : PCA+ A) where
  ...
  record Conᴿ (ΓLᴿ : ConLᴿ) (ΓCᴿ : ℕ) : Set1 where
    field
      coh : ΓCᴿ ≡ ΓLᴿ .len
      
  open Conᴿ

  record SubLᴿ (ΓLᴿ ΔLᴿ : ConLᴿ) : Set where
    field
      ∣_∣ : ∣ ΓLᴿ ∣ → ∣ ΔLᴿ ∣
      tracker : ∣ A [ ΓLᴿ .len ]∣^ (ΔLᴿ .len)
      _ᵀᴿ : Tracked ∣_∣ tracker (ΓLᴿ ᴿᴿ) (λ _ → ΔLᴿ ᴿᴿ)
...
--   ∣ RL .TT-Logic.Λ ∣ γ = {!   !}
--   ∣ RL .TT-Logic.Λ ∣⁺ γ a = ⊥
--   (RL .TT-Logic.Λ ᴿᴿ) γ a (r , u) = ⊥-elim u
--   RL .TT-Logic.Λ .total γ (r , u) = ⊥-elim u
--   RL .Λ[] = refl
--   RL .lamΛ x γ = extract (Λ' {!   !})
--   RL .appΛ x y γ = {!   !} -- x γ ∙' y γ
