...
module OverPCA+ (A : PCA) (A+ : PCA+ A) where
  ...
  record Conᴿ (ΓLᴿ : Set) (ΓCᴿ : ConC synC) : Set1 where
    field
      ∣_∣ : ΓLᴿ → Set
      _ᴿᴿ : RRel (size ΓCᴿ) (Σ[ γ ∈ ΓLᴿ ] ∣_∣ γ)
      total : Total _ᴿᴿ
      -- cart : ΓCᴿ -Cart _ᴿᴿ
  ...
  record Tmᴿ {ΓLᴿ ΓCᴿ}
    ...
    (Tᴿ : Tyᴿ ΓLᴿ)
    (aLᴿ : (γ : ΓLᴿ) → ∣ Tᴿ ∣ γ)
    (aCᴿ : TmC synC ΓCᴿ) : Set where
    field
      ∣_∣ : ∀ γ (γ' : ∣ Γᴿ ∣ γ) → ∣ Tᴿ ∣⁺ γ (aLᴿ γ)
      _ᵀᴿ : ∃Tracked (λ (γ , γ') → (aLᴿ γ , ∣_∣ γ γ')) (Γᴿ ᴿᴿ) (λ (γ , γ') → (Tᴿ ᴿᴿ) γ)
      interp : ∀ γ γ' → Σ[ a ∈ ∣ A ∣^ 1 ] {!   !}

-- ∀ (γ : _) (γ' : ∣_∣ ) → (a : ∣ A ∣^ (size )) → a ⊩[ Γᴿ ᴿᴿ ] x → (fR ∙* a) !⊩[ (Tᴿ ᴿᴿ) γ x ] y

  -- interp : ∀ {ΓC} → TmC synC ΓC → ∣ A [ size ΓC ]∣
  -- interp ? = ?
